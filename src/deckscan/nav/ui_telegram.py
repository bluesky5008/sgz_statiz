...
# 우상단 '귀환' — 상세 화면 등에서 한 단계 되돌아가는 공용 버튼. 순회 중
# 화면 이탈 복구에 사용(2026-08-09 실기: 열람된 단건 행 클릭은 인라인 펼침이
# 아니라 전투 상세 화면으로 전환됨 — 결함 F).
CLICK_RETURN = (2499, 24)
...
MOUSE_PARK = (400, 350)
# 접힌 행 펼침 클릭 지점 — 동맹명과 공격 유저명 사이의 넓은 빈 구간.
# 실측(2026-08-09, cmd_0004~0005): 우측 화살표(∨) 열(x=1771)은 클릭 무효,
# 빈 구간 x=990·1336은 즉시 펼침. 990 채택 — 단건 행의 중앙 위치 링크
# (좌표 자릿수에 따라 폭 가변)·유저명·동맹명에서 가장 먼 지점(오클릭 시
# 월드맵 이동 — 2026-08-09 실기 사고, DCR-001).
ROW_CLICK_X = 990
