...
- 현재 결론 또는 상태: **TASK-01~12·14 완료**(기준선 v5 = DCR-001~004). 실기 결함 A~F 전부 실측·수정, AC-01~06 검증 성공·AC-07 부분 성공([검증 결과](#검증-결과-verification)). 오프라인 테스트 **61건 성공**. 잔여: TASK-13(README·완료 보고), AC-07 대화형 UX 사용자 확인.
- 다음 행동: TASK-13 문서화 → 완료 보고.
...
- 진행 중인 작업: TASK-13 (README 최종화·완료 보고)
- 마지막 완료 작업: TASK-12 완료 — AC-01~07 판정·검증 기록(2026-08-09, 테스트 61건)
- 작업 트리 상태: 구현·검증 13/14 완료 — [plan.md](./plan.md#계획-트리)
- 차단 요인: 없음. 실행기 운영 메모: 재기동 시 done 마커 스킵(agent_shell.ps1 수정 적용 확인 — 2026-08-09 18:41 세션이 seq 1~9 스킵). **실기 대상 창은 매 세션 probe로 재확인**(현 세션 `--hwnd 0x508b4`=羊커리 — 0x60140 컬러단장용스는 사용 금지). 실행기 경유 deckscan 명령은 반드시 --hwnd 지정(실행기 콘솔은 진짜 TTY라 선택 프롬프트가 블록됨).
...
### 2026-08-09 — DCR-002 승인(기준선 v3), TASK-14 클라이언트 창 선택 (TDD + 부분 실기)

- **DCR-002 발의·승인**: 사용자 지시 — "羊커리만"은 테스트 한정 제약이며, 운영에서는 구동 시 클라이언트를 동적 탐지해 사용자 선택을 받아야 함. 사용자 결정 3건(선택 UX=목록+전면 확인, 단일 후보=자동 진행, 구현 시점=이번 작업 TASK-14) 반영해 [DCR-002](./changes/DCR-002-client-selection.md) 승인, FR-08·AC-07 신설, REQ·DESIGN **기준선 v3** 발행, plan에 TASK-14 편성(TASK-13이 12·14에 의존).
- **TASK-14 구현(TDD Red→Green)**: `controller.choose_window`(후보 1개 자동 반환 / 다중 번호 선택 → `raise_to_top` 전면 표시 → y/n 확인 / q·EOF 중단, 입력 콜백 주입) + `cli._resolve_session`(scan·probe 공통 창 확정 — `--hwnd` 생략 경로, 비대화형 다중 거부). 선행 테스트 6건.
- **실기 발견·수정(TDD)**: Windows에서 stdin이 NUL 장치면 `isatty()`가 True를 반환해 비대화형 가드가 뚫리고 input()이 EOFError 트레이스백으로 사망 — EOF=중단(None) 처리 추가(재현 테스트 선작성).
- **실기 확인(부분)**: ① 비대화형 다중 후보 → 후보 목록 출력 후 exit 1(트레이스백 없음) ✓, ② `--hwnd` 생략 경로 ✓. **클라이언트 2대 실행 중 식별**: `0x508b4`(pid 13608, 우측 모니터) = **羊커리**(군주명 스냅샷 확인, 메인 화면, 전보 배지 130) / `0x60140`(pid 8528, 좌측) = 컬러단장용스(사용 금지). 지난 세션 hwnd 0x2097c는 클라이언트 재시작으로 무효. 단일 후보 자동 진행 스모크는 상시 2대 환경이라 불가 — 단위 테스트+다중 실증으로 갈음. **대화형 선택 UX 실기(AC-07)는 TASK-12 검증에 편입**(사용자 터미널에서 수행 가능).
...
기준선 v5(DCR-001~004 반영) 기준. 오프라인 테스트 **61건 전체 성공**(2026-08-09). 실기 검증은 관리자 실행기 경유(羊커리 클라이언트 0x508b4).

| ID | 인수 조건 | 방법 | 결과 | 증거 |
|---|---|---|---|---|
| VER-01 | [AC-01](./requirements.md#인수-조건) 메인→교전 탭 자동 도달 | 실기 — scan 초기 내비게이션(재시도 로직 실전 작동 포함) | **성공** | res_0010·0011.log("교전 탭 도달"), output/t12/combat.png, res_0015(전일) |
| VER-02 | [AC-02](./requirements.md#인수-조건) img/4 기준값 추출 | 오프라인 인수 테스트(`test_ac02_img4_record_and_label_lookup`) — 유저·장수·레벨·라벨 조회 | **성공** | tests/test_deck_parser.py, 61건 스위트 |
| VER-03 | [AC-03](./requirements.md#인수-조건) N건 저장·재실행 불변 | 실기 연속 2회(`--no-scroll`, DB 리셋 후): run A 5건 저장 → run B 동일 조건 재실행이 4건을 재파싱해 **동일 키 재현** → 총 5건 불변·중복 그룹 0·불완전 덱 0. 보조: 전체 순회 run(37처리→16고유, 중복 0) + 키 단위 테스트(재현·병력/레벨 불변·멱등) | **성공** | res_0010·0011.log, runs 테이블(5/5·4/4), ac03 검사 스크립트 출력, tests/test_datastore.py |
| VER-04 | [AC-04](./requirements.md#인수-조건) 부분 실패 계속+요약 보고 | 실기(전체 순회 run: partial 1건 — battle_time 실패·증거 크롭 저장·순회 계속·요약 출력) + 오프라인 훼손 주입 테스트 | **성공** | res_0008.log(요약), output 증거 크롭, tests/test_deck_parser.py |
| VER-05 | [AC-05](./requirements.md#인수-조건) (전보,진영,슬롯) 조회 구조 | 단위(deck_rows 조인) + 실기 export 스모크(CSV 3종 — battles·deck_long·generals) | **성공** | tests/test_controller.py, output/export/*_20260809.csv |
| VER-06 | [AC-06](./requirements.md#인수-조건) pending 식별자·라벨 확정 반영 | 단위(label 흐름: 확정·건너뜀·중단·확정 후 조회 반영). 실기 대화는 운영 절차(비대화형 실행 불가 — 사용자 터미널에서 `deckscan label`) | **성공** (실기 대화는 운영 시 수행) | tests/test_controller.py·test_deck_parser.py |
| VER-07 | [AC-07](./requirements.md#인수-조건) 다중 클라이언트 선택 (v3) | 단위 6건(단일 자동·선택·확인 거부·중단·오입력·EOF) + 실기 부분(2클라이언트 환경에서 비대화형 다중 거부·`--hwnd` 경로 실증) | **부분 성공** — 대화형 선택 UX의 실기 확인은 사용자 터미널 필요(잔여) | tests/test_controller.py, res_0001·0002.log(신규 세션) |

비기능: NFR-01(부분 실패 계속 — VER-04), NFR-02(상태 기반 대기 — wait_stable·마커, 재시도 실전 작동 다수), NFR-03(capture_path·증거 저장 — 단위+실기 파일 확인), NFR-04(초기화 해상도 검증 — scan 기동 시 2544×657 확인) 충족.

미수행·잔여: ① AC-07 대화형 선택 실기(사용자 확인 — `.\.venv\Scripts\python.exe -m deckscan.cli probe` 터미널 실행으로 가능) ② AC-02의 수비 슬롯2 라벨 후보 '성채'는 TBD — `label` 절차에서 사용자 확정 ③ 열람(읽음) 처리된 전보는 재수집 불가(운영 한계 — DB를 유지하는 한 문제 없음, 초기화 시 과거 전보 유실) ④ 병력 값 오독은 키와 무관하게 잔존 가능(캡처로 감사, NFR-03).
...
- 없음 — TASK-01~14 전체 완료. 잔여 위험·후속은 [완료 보고](#완료-보고-completion) 참조.

## 완료 보고 (completion)

- **완료 상태: 완료** (2026-08-09, 기준선 v5 = DCR-001~004). TASK-01~14 전체 완료.
- 완료한 내용: deckscan CLI 전체 — 창 동적 탐지·선택(FR-08), 메인→교전 탭 내비게이션(FR-01), 전보 목록 순회·펼침·복구(FR-02), 덱 파싱(FR-03), 완전 덱 저장·멱등(FR-04·05, A-03 v2), 요약(FR-06), pending 라벨 운영(FR-07), CSV 3종 내보내기.
- 주요 변경: [수행 기록](#수행-기록) 참조. 실기 검증에서 결함 A~F를 실측·수정했고, 그중 3건은 설계 변경(DCR-002 창 선택, DCR-003 battle_key 결정적 재료, DCR-004 완전 덱 게이트)으로 재승인받았다.
- 설계와 달라진 점: [해당 절](#설계와-달라진-점) — 경미 3건(기록됨), 중대 변경은 전부 DCR로 처리.
- 검증 결과: [검증 결과](#검증-결과-verification) — AC-01~06 성공, AC-07 부분 성공(대화형 UX 사용자 확인 잔여). 오프라인 테스트 61건 성공. 실행 명령·증거는 VER 표에 기재.
- 통합 상태: main 브랜치 로컬 커밋 완료(작업 트리 클린). push는 사용자 지시 대기.
- 남은 위험·제한: ① 열람 전보 재수집 불가(운영 한계 — DB 유지 필수, README 문서화) ② 병력 값 OCR 오독 잔존 가능(키·중복 무관, 캡처 감사 NFR-03) ③ 동일 장수 구성·동시·동일쌍 전투 병합(DCR-003 한계) ④ 레벨 글리프 6·7 미수확 ⑤ row_icon의 동맹 문장 여부 미확정(동맹 변경 시 재검증) ⑥ 유저·동맹 NCC 임계 0.80은 대규모 데이터로 재보정 여지(현행 문제 미관측) ⑦ SCROLL_NOTCHES 미세 보정 여지(현행 종착 판정 정상).
- 후속 작업: AC-07 대화형 실기 확인(사용자 터미널 `deckscan probe`), `label`로 pending 라벨 확정 운영, 포트폴리오 작업 2(교전 통계 분석·시각화 — [PF-sgz-statiz](../PF-sgz-statiz/portfolio.md)).

## 재개 지점

- 작업 완료 — 재개 없음. 운영 참고: 실기 세션마다 `probe`로 대상 창 hwnd 재확인(클라이언트 재시작 시 변동), 실행기 경유 deckscan 명령은 `--hwnd` 필수(실행기 콘솔이 TTY라 선택 프롬프트가 블록됨). 다음 작업 후보는 포트폴리오 작업 2(통계 분석·시각화).
