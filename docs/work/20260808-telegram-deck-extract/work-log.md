...
- 현재 결론 또는 상태: **TASK-01~11·14 완료**(기준선 v3, DCR-002 — FR-08 창 선택 포함). TASK-12 진행 중 — cmd_0018 분석으로 실기 결함을 재분류(결함 B~E)하고 **오프라인 수정 전부 완료**(OCR 오독 3종, 무효 렌더 게이트, 상단 경계 가드, 마우스 파킹, 정지 화면 오판). 오프라인 테스트 **57건 성공**. 남은 것: 결함 A(묶음 행 펼침) 실측과 AC-01~07 실기 검증 — 관리자 실행기 재기동 필요(유휴 300분 초과로 종료됨). 실기 대상 창 `--hwnd 0x508b4`(羊커리).
